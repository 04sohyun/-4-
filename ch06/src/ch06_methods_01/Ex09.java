...
public class Ex09 {
	public static void main(String[] args) {
		//default생성자 없으면 default생성자 호출 불가
		//매개변수 3개짜리 생성자 호출하여 객체 생성
		B3 b = new B3(98,89,93);
		b.sum();//메소드호출
		System.out.println("합:"+b.sum);//필드접근
...
class B3{
	...
	//매개변수X, 리턴타입X
	void sum() {
		sum = kor + eng + mat;//sum()메소드에 의해 sum=0으로 초기화된 상태에서 kor+eng+mat결과값으로 갱신
	}
	//매개변수X, 리턴타입O
	int average() {
