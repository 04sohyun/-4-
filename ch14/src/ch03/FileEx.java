...
public static void main(String[] args) {
	...
	File f2 = new File("c:\\temp1\\java_sample");
	//파일 존재여부 확인 exists()
	if(!f2.exists()) {//파일정보가 없으면
		f2.mkdir();//생성 - 디렉토리로 생성 mkdir()
	}
	//내용 출력
	listDirectory(new File("c:\\temp1"));
	
	//디렉토리명 변경 - 파일명 변경 rename(새로운파일정보)
	f2.renameTo(new File("c:\\temp1\\javasample"));
...
public static void listDirectory(File dir) {
	...
	for(int i=0;i<subFiles.length;i++) {
		File f=subFiles[i];
		long t = f.lastModified();//생성된 일시리턴(수정된 일시), 1970,1,1,0,0,0
